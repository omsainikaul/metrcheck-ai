...
async def _resolve_snapshot(version_id: str, version_type: str = "ANALYSIS") -> VersionSnapshot:
    """Helper to resolve a VersionSnapshot from either analyses or artworks table."""
    ...
    if v_type == "ARTWORK" or version_id.startswith("art-"):
        ...
        comp_res_dict = ana_res.get("compliance_result") or {}
        comp_res = ComplianceResult(**comp_res_dict) if comp_res_dict else None
        
        score = float(ana_res.get("overall_score", 0.0))
        risk_level = "LOW" if score >= 90 else ("CRITICAL" if score < 60 else "MEDIUM")

        preview_url = None
        pages = art.get("pages_data", [])
        if pages and isinstance(pages, list) and len(pages) > 0:
            preview_url = pages[0].get("preview_image_path") or pages[0].get("image_url")

        return VersionSnapshot(
            version_id=art["id"],
            version_label=f"{art['filename']} (v{art.get('iteration_number', 1)})",
            version_type="ARTWORK",
            product_name=p_info.product_name or art["filename"],
            ...
            parent_id=art.get("parent_artwork_id")
        )
    else:
        ...
        ext_data = analysis.get("extracted_data")
        if isinstance(ext_data, str):
            try: ext_data = json.loads(ext_data)
            except Exception: ext_data = {}
        p_info = ProductInfo(**ext_data) if ext_data else ProductInfo(product_name=analysis.get("product_name"))
        ...
        image_url = images[0].get("image_url") if images and len(images) > 0 else f"/uploads/{analysis.get('image_filename')}"
...
async def compare_two_versions(req: VersionComparisonRequest):
    """
    ...
    """
    snap_a = await _resolve_snapshot(req.version_a_id, req.version_type_a)
    snap_b = await _resolve_snapshot(req.version_b_id, req.version_type_b)

    result = compare_versions(snap_a, snap_b)
    await save_version_comparison(result.model_dump())
    return result
...
async def list_recent_comparisons(
    ...
    """List saved version comparisons."""
    comps = await list_version_comparisons(owner_user_id=owner_user_id, limit=limit)
...
async def get_comparison_details(comparison_id: str):
    """Retrieve details for a specific version comparison."""
...
async def export_comparison_json(comparison_id: str):
...
async def export_comparison_csv(comparison_id: str):
...
async def get_entity_version_timeline(entity_id: str):
    """
    Retrieve chronological version timeline events for a product name or artwork chain.
    """
    events = await get_version_timeline(entity_id)
...
async def list_comparable_version_targets(
    ...
    for Version A and Version B selection.
    """
    ...
    # Add Analyses
    for a in analyses:
        if owner_user_id and a.get("owner_user_id") and a.get("owner_user_id", "").lower() != owner_user_id.lower():
            continue

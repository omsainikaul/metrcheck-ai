    ...
    <div className="space-y-6 animate-in fade-in duration-300">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                Manage registered business SKUs, master package declarations, and compliance lifecycle records.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Active SKUs</span>
        ...
        <div className="p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Screened Checks</span>
        ...
        <div className="p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Attention Required</span>
        ...
        <div className="p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Pre-Print Artworks</span>
      ...
      <div className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-2xs flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search by Product name, Brand, GTIN / Barcode, FSSAI..."
            value={searchQuery}
      ...
      {filteredProducts.length > 0 ? (
            ...
            <table className="w-full text-left text-sm border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-800 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                  <th className="py-3 px-4 font-semibold">SKU / Product Info</th>
                  <th className="py-3 px-4 font-semibold">Category</th>
                  <th className="py-3 px-4 font-semibold">GTIN / Barcode</th>
                  <th className="py-3 px-4 font-semibold">Declared MRP &amp; Net Qty</th>
                  ...
                  >
                      ...
                      <div>
                        ...
                        {p.brand_name && (
                          <div className="text-xs text-slate-500 dark:text-slate-400">
                            Brand: <span className="font-medium text-slate-700 dark:text-slate-300">{p.brand_name}</span>
                          </div>
                        )}
                        {p.manufacturer_name && (
                    ...
                    <td className="py-3 px-4 text-right">
                      <div className="inline-flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                        <button
                          onClick={() => navigate(`/products/${p.id}`)}
                          title="View Product Details & History"
        ...
        <div className="p-12 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl">
          <EmptyState
            icon={Boxes}
            title={searchQuery ? "No matching products found." : "No registered products yet."}
            description={searchQuery ? "Try refining your search terms or filter criteria." : "Create your first product master record to link physical scans, pre-print packaging artworks, and monitor end-to-end compliance."}
            actionLabel={searchQuery ? undefined : "Add First Product SKU"}
            onAction={searchQuery ? undefined : () => navigate('/products/new')}
          />
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
            ...
            <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
              Archiving sets this SKU to inactive status. All historical scan records, pre-print packaging artworks, and compliance inspection certificates remain securely preserved for audit integrity.
            </p>
